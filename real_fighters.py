...
killerVirusImage = pygame.image.load('killerVirus.png')
...
while True:
	...
	while True:
		...
		# add killer virus 
		killerVirusAddCounter += 1
		if killerVirusAddCounter == ADDKILLERVIRUSRATE:
			killerVirusAddCounter = 0
		...
		for b in DocsMedicines:
			for kv in killerVirus:
				if b.colliderect(kv['rect']):
					kv['hit'] -= 1
					# check medicines have hit the killer virus 3 times
					if kv['hit'] > 0:
						DocsMedicines.remove(b)
						break
					else:
